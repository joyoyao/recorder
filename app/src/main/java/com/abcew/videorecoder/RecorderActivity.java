		...
		new AsyncTask<String, Integer, Boolean>(){
			...
			protected void onPostExecute(Boolean result) {
				...
				if(recorderThread == null) {
					recorderThread = new RecorderThread(  previewHeight * previewWidth * 3 / 2,frameRate*(RECORDING_TIME /1000));
				...
				topLayout.setOnTouchListener(RecorderActivity.this);
...
//		}
	...
	public class AsyncStopRecording extends AsyncTask<Void,Integer,Void> {
		...
		protected void onPreExecute() {
			...
			//创建处理进度条
			creatingProgress= new Dialog(RecorderActivity.this,R.style.Dialog_loading_noDim);
...
//			videoRecorder = null;
	...
	private void showCancellDialog(){
		Util.showDialog(RecorderActivity.this, "提示", "确定要放弃本视频吗？", 2, new Handler(){
			@Override
			public void dispatchMessage(Message msg) {
				if(msg.what == 1)
					videoTheEnd(false);
...
//		yuvIplImage = IplImage.create(previewHeight, previewWidth,IPL_DEPTH_8U, 2);
		...
		if(Build.VERSION.SDK_INT >  Build.VERSION_CODES.FROYO)
		{
			mCamera.setDisplayOrientation(Util.determineDisplayOrientation(RecorderActivity.this, defaultCameraId));
